...
public class Collectd {
	...
	public void send(String name, long value, long timestamp, DataType type, long period) {
		int commaIndex = name.lastIndexOf('.');

		if (type.equals(DataType.COUNTER)) {
			if (commaIndex == -1) {
				send(name, "0", value, timestamp, type, period);
			} else {
				send(name.substring(0, commaIndex), name.substring(commaIndex + 1), value, timestamp, type, period);
			}
		} else { // gauge
			if (commaIndex == -1) {
				send(name, "0", (double) value, timestamp, type, period); // java 7 magic
			} else {
				send(name.substring(0, commaIndex), name.substring(commaIndex + 1), (double) value, timestamp, type, period);
			}
		}
	}

	public void send(String name, double value, long timestamp, DataType type, long period) {
		if (!type.equals(DataType.GAUGE)) {
			throw new RuntimeException("Double must be gauge!");
		}
	...
	public void send(String name, Number value, long timestamp, DataType type, long period) {
	...
	public void send(String name, String typeInstance, long value, long timestamp, DataType dataType, long period) {
		sendPacket(name, typeInstance, value, timestamp, dataType, period);
	}

	public void send(String name, String typeInstance, double value, long timestamp, DataType dataType, long period) {
		sendPacket(name, typeInstance, Double.doubleToRawLongBits(value), timestamp, dataType, period);
	}

	private void sendPacket(String name, String typeInstance, long value, long timestamp, DataType dataType, long period) {
		...
		sendPacket(plugin, pluginInstance, typeInstance, value, timestamp, dataType, period);
	}

	private void sendPacket(String plugin, String pluginInstance, String typeInstance, long value, long timestamp, DataType dataType, long period) {
		...
		// gauge is littleEndian
		boolean bigEndian = dataType.equals(DataType.COUNTER);

		try {
			writeString(buffer, Network.TYPE_HOST, hostname);
			writeNumber(buffer, Network.TYPE_TIME, timestamp);
		...
		if (buffer.size() > Network.BUFFER_SIZE) {
			LOGGER.warn("discard metric: {} size exceed: {}", plugin, buffer.size());
			return;
		}
